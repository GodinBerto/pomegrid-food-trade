...
export default function AdminOrders() {
    const { data: orders = [] as any[], isLoading } = useAdminListOrders();
                ...
                {orders.map((o: any) => (
                    <div key={o.id} className="rounded-3xl bg-muted p-5">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                            <div>
                                <div className="font-semibold">Order #{o.id.toString().slice(0, 8)}</div>
                            ...
                            <select
                                ...
                                onChange={async (e) => {
                                    try {
                                        await updateOrder({ id: o.id.toString(), status: e.target.value });
                        ...
                        {o.notes && <div className="mt-2 rounded-2xl bg-background p-3 text-sm">📝 {o.notes}</div>}
                    </div>
                ))}
                {orders.length === 0 && <div className="rounded-3xl bg-muted p-10 text-center text-muted-foreground">No orders yet.</div>}
            </div>
        </div>
    );
}
