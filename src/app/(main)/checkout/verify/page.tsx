...
export default function VerifyPaymentPage() {
    ...
    const [status, setStatus] = useState<"verifying" | "placing_order" | "success" | "error">("verifying");
    ...
    useEffect(() => {
        ...
        async function processPayment() {
            try {
                ...
                // Assuming success means status is 'success' or similar in the paystack response
                if (!res.ok) {
                    throw new Error(res.message || "Payment verification failed");
                }

                const paymentData = res.data;
                // Double check the payment status from our DB response
                if (paymentData.status !== "success") {
                     throw new Error(`Payment is not successful yet. Status: ${paymentData.status}`);
                }
                ...
                if (!savedOrder) {
                    throw new Error("Order details not found. Please contact support if you were charged.");
                }
            ...
            {status === "error" && (
                <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="mt-4 flex gap-4">
                        <Link href="/checkout" className="rounded-full bg-primary px-6 py-2 text-sm font-semibold text-primary-foreground">
                            Try Again
                        </Link>
                        <Link href="/contact" className="rounded-full bg-secondary px-6 py-2 text-sm font-semibold text-secondary-foreground">
                            Contact Support
                        </Link>
                    </div>
                </div>
