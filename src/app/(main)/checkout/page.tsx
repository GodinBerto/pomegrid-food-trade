    ...
    if (items.length === 0) {
        return (
            <div className="mx-auto max-w-2xl px-4 py-20 text-center">
                ...
                <Link href="/shop" className="mt-4 inline-block rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground">Browse products</Link>
            </div>
        );
    }
            ...
            <form onSubmit={onSubmit} className="mt-8 grid gap-8 lg:grid-cols-[1fr_360px]">
                <div className="space-y-4 rounded-3xl bg-muted p-6">
                    ...
                    <div>
                        ...
                        <input required value={phone} onChange={(e) => setPhone(e.target.value)} className="mt-1 w-full rounded-2xl bg-background px-4 py-3 text-sm" placeholder="+233…" />
                    </div>
                    ...
                    <div>
                        ...
                        <select
                            value={deliveryType}
                            onChange={(e) => setDeliveryType(e.target.value as "pickup" | "standard")}
                    ...
                    <div>
                        ...
                        <select value={region} onChange={(e) => setRegion(e.target.value)} className="mt-1 w-full rounded-2xl bg-background px-4 py-3 text-sm">
                            {regionList.map((r) => <option key={r} value={r}>{r}</option>)}
                        </select>
                        <p className="mt-1 text-xs text-muted-foreground">Only regions we currently serve are listed.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} className="mt-1 w-full rounded-2xl bg-background px-4 py-3 text-sm" placeholder="Anything we should know?" />
                    </div>
                </div>
                ...
                <aside className="h-fit rounded-3xl bg-primary p-6 text-primary-foreground">
                    ...
                    <p className="mt-3 text-xs opacity-80">
                        Your order is placed now. You'll be guided through secure payment and delivery next.
                    </p>
                </aside>
            </form>
        </div>
