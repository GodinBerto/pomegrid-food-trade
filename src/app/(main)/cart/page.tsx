...
export default function CartPage() {
    const { items, setQty, remove, total, hydrated, clear, isUpdating, isLoggedIn } = useCart();

    if (!hydrated) {
        return <div className="mx-auto max-w-4xl px-4 py-14 lg:px-8">Loading cart…</div>;
    }
    ...
    if (!isLoggedIn) {
        return (
            <div className="mx-auto max-w-2xl px-4 py-20 text-center">
                ...
                <p className="mt-2 text-muted-foreground">Your cart is saved to your account once you sign in.</p>
        ...
        <div className="mx-auto max-w-6xl px-4 py-10 lg:px-8">
            ...
            <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_360px]">
                <div className="space-y-3">
                    {items.map((it) => (
                        <div key={it.id} className="flex items-center gap-4 rounded-2xl bg-muted p-4">
                            <div className="h-16 w-16 overflow-hidden rounded-xl">
                                <img src={it.imageUrl || categoryImage(null)} alt={it.name} className="h-full w-full object-cover" />
                            </div>
                            <div className="flex-1">
                                <div className="font-semibold">{it.name}</div>
                                <div className="text-xs text-muted-foreground">{formatGHS(it.pricGhs)} each</div>
                                {!it.isActive && (
                                    <div className="mt-1 text-xs font-medium text-destructive">Currently unavailable</div>
                                )}
                            </div>
